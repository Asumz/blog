...
:::info 提示
由于子应用是通过`webcomponent`渲染的，所以子应用弹窗可以突破页面限制
:::
...
receiptsHandler.show({ /** 业务参数 */ })

receiptsHandler.afterConfirm(data => { /** 回调参数 */ })
```
...
```js
// src/pages/Receipt/CreateReceipt
// 提交操作成功后
eventBus.emit(eventBusCode.afterReceiptsDrawerConfirm, data);
```
...
receiptsDrawerHandler.setConfig({ /** Drawer属性 */ })

receiptsDrawerHandler.show({ /** 业务参数 */ })

receiptsDrawerHandler.afterConfirm(data => { /** 回调参数 */ })
```
