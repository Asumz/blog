...
所以该实现方案限制 Wujie 难以由`webcomponent`降级到`iframe`
:::
...
const online = process.env.MUJI_APP_FINANCE_PROJECT
...
export const useReceiptsDrawer = () => {
    ...
    const instance = useMemo(() => {
        if (subAppWindow) return subAppWindow?.receiptsDrawerHandler
    }, [subAppWindow])
    ...
    const show = (/** @type {ReceiptsDrawerProps} */ data) => {
        subAppWindow?.receiptsDrawerHandler.show(data)
    }
    ...
    const afterConfirm = (/** @type {(data: any) => void} */ cb) => {
        subAppWindow?.receiptsDrawerHandler.afterConfirm(cb)
    }
