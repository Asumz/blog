...
-   方案 2：直接在子应用 B 中将表单页改造为弹窗，通过无界`bus`事件跨项目通信。优点为没有业务迁移成本，只在原有基础上迭代业务即可，尽可能保留 Git 记录。开发过程中发现主项目 A 中可能会同时创建多个 Wujie 实例，采用`bus`通信产生了一些问题，主项目 emit 事件，会被多个实例同时监听，产生难以预料的 bug，遂放弃~
...
所以该实现方案限制 Wujie 无法由`webcomponent`降级到`iframe`
:::
...
const realOrigin = `${F2ES_url_new}/projects/Mars_WEB/mars-web-finance`
const dealOrigin = origin.includes('localhost') ? localOrigin : realOrigin

const subAppName = 'finance-blank'
const subAppUrl = dealOrigin + '/#/blank'

const HiddenSubApp = ({ setReady, setHandler }) => {
    return (
        <WujieReact
            width="0"
            height="0"
            ...
            afterMount={appWindow => {
                setReady(true)
                setHandler(appWindow.receiptsDrawerHandler)
            }}
        />
    )
}
...
export const useReceiptsDrawer = () => {
    const [ready, setReady] = useState(false) // 实例创建后可以调用子应用方法
    ...
    useEffect(() => {
        ...
        ReactDom.render(
            <HiddenSubApp
                setReady={setReady}
                setHandler={newHandler => (handler.current = newHandler)}
            />,
            div
        )
        return () => {
    ...
    const show = (/** @type {ReceiptsDrawerProps} */ data) => {
        handler.current.show(data)
    }

    const hide = () => {
        handler.current.hide()
    }

    const afterConfirm = (/** @type {(data: any) => void} */ cb) => {
        handler.current.afterConfirm(cb)
    }
