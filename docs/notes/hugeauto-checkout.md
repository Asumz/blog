...
接入 PayPal 和 Antom 后，业务页面既要知道用户购买了什么，又要知道不同渠道如何创建支付、如何加载 SDK、如何解释回调。我最初想把支付区域抽成公共组件，后来发现这只能复用界面，订单有效期、渠道可用性、支付状态和失败重试仍然会留在业务组件中。

因此我把完整支付过程拆成了统一收银台。业务页面只负责创建业务订单，后端签发 Checkout Session，收银台负责选择支付方式与拉起渠道，结果页负责确认最终状态。

设计状态模型时，我发现只用 `S/F/U` 仍然存在一个关键歧义：同样是 `U`，用户可能还没有点击支付，也可能已经拉起渠道、正在等待异步结果。这两种状态显然不能执行同一种操作。

## 整体流程从 U 的两个阶段展开

整体设计保留 `S/F/U` 作为支付主状态，同时使用 `paymentStatusPhase` 细分 `U`：
...
这次没有继续给 `S/F/U` 增加更多主状态，而是把“状态”和“阶段”分开。`S/F/U` 回答支付最终处于什么状态，`paymentStatusPhase` 只回答 `U` 当前走到了哪一步。
...
这个状态门槛消除了 `U` 的歧义：主状态只能说明“尚未成功或失败”，Phase 则进一步决定能否继续创建支付。最终，创建权限被明确限制在 `U + NOT_STARTED`。
...
PayPal 和 Antom 的交互差异仍然保留：PayPal 接口返回 `paypalOrderId`，交给 Smart Buttons；Antom 接口返回 `paymentSessionData`，交给 SDK。我没有为了形式统一而合并两个创建接口，只统一了真正稳定的请求边界。
...
我曾考虑过前端等待超时后直接合成失败，但这可能出现用户已经扣款、页面却显示失败的情况。宁愿暂时保留 `U + PROCESSING`，也不能给出未经后端确认的结论。
...
回头看，这套统一收银台最终解决的是“支付逻辑放在哪里”以及“每一步由谁决定”：
...
我认为最关键的变化不是增加了几个字段，而是消除了 `U` 的歧义。只有把“尚未开始”和“正在确认”拆开，收银台才能明确判断此刻应该允许支付、拒绝重复创建，还是安静地等待最终结果。
