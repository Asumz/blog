...
| 方案名称     | 实现方式                                       | 优势                                       | 劣势                     |
| ------------ | ---------------------------------------------- | ------------------------------------------ | ------------------------ |
| 主应用弹窗   | 将子应用业务模块迁移至主应用，由主应用实现弹窗 | • 技术实现简单                             | • 重构量大<br>• 业务混淆 |
| 组合式弹窗   | 将子应用内容嵌入主应用弹窗容器                 | • 重构量小<br>• 业务解耦                   | • 子应用无法独立调试     |
...
弹窗调用以 hooks 模式设计，需要创建 HOC 兼容 class 组件

### 4. 子应用未准备好时弹窗无法调起怎么处理？
